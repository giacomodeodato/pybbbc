...
class bbbc021:
    ...
    def make_dataset(file_path):
        ...
        def extract_plate(plate):
            images_dir = os.path.join(bbbc021.data_dir, 'raw', 'images')
            filename = [
                x
        ...
        raw_data_dir = os.path.join(bbbc021.data_dir, 'raw')
        hdf5_dir = os.path.join(bbbc021.data_dir, 'hdf5')
    ...
    @staticmethod
    def _download_raw_data(data_path=None):
        ...
        bbbc021.data_dir = os.path.join(data_path, 'data')
        if not os.path.exists(bbbc021.data_dir):
            os.mkdir(bbbc021.data_dir)
        raw_data_dir = os.path.join(bbbc021.data_dir, 'raw')
        if not os.path.exists(raw_data_dir):
            os.mkdir(raw_data_dir)

        urls_path = download_file(urls_metadata, dst_dir=raw_data_dir)
        ...
        if not os.path.exists(images_dir):
            os.mkdir(images_dir)
        urls_path = download_file(urls_images, dst_dir=raw_data_dir)
        with open(urls_path) as file_object:
